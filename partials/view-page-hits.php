	...
	*/
	if ( isset ( $_GET['reset_page'] ) && isset( $_GET['page_id'] ) && wp_verify_nonce( $_GET['nonce'], 'delete_reset_deny' ) ):
		...
		if ( WHTP_Hits::reset_page_count( $page_id ) ) : ?>
			<div class="notice notice-success">
                <p>
                    <?php _e('The count for the page has been reset successfully.', 'whtp'); ?>
                </p>
            </div><?php		
		else: ?>
    ...
    if ( isset( $_GET['delete_page'] ) && isset( $_GET['page_id'] ) && wp_verify_nonce( $_GET['nonce'], 'delete_reset_deny' ) ):
        ...
        if ( WHTP_Hits::delete_page( $page_id ) ): ?>
			<div class="notice notice-success">
				<p>
                    <?php _e( 'Page count deleted. New entries will be made when users visit your pages again. If you no longer wish to count visits on certain pages, go to the page editor and remove the <code>[whohit]..[/whohit]</code> shortcode.', 'whtp' ); ?>
				</p>
			</div><?php
		else: ?>
			<div class="notice notice-warning">
