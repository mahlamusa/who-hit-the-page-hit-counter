...
if ( isset( $_GET['delete_ip'] ) && isset( $_GET['ip_address'] ) && wp_verify_nonce( $_GET['nonce'], 'delete_reset_deny' ) ) :
	...
	?>
				<div class="notice notice-success">
					<p><?php es_attr_e( 'IP addresse(s) removed from the database.', 'whtp' ); ?></p>
				</div>
			<?php
		else :
		?>
				<div class="notice notice-warning">
					<p><?php es_attr_e( 'Failed to remove IP addresse(s) from database', 'whtp' ); ?></p>
				</div>
			<?php
		endif;
	endif;
...
if ( isset( $_GET['deny_ip'] ) && isset( $_GET['ip_address'] ) && wp_verify_nonce( $_GET['nonce'], 'delete_reset_deny' ) ) :
	...
	?>
			<div class="notice notice-success">
				<p><?php es_attr_e( 'The IP Address will be ignored and will not be counted.', 'whtp' ); ?></p>
			</div>
		<?php else : ?>
			<div class="notice notice-warning">
				<p><?php es_attr_e( 'Failed to add IP to ignore list', 'whtp' ); ?></p>
			</div>
		<?php
		endif;
	endif;
...
if ( isset( $_POST['reset_all_ips'] ) && wp_verify_nonce( $_POST['reset_all_ips_nonce'], 'reset_all_ips' ) ) :
	...
	?>
			<div class="notice notice-success">
				<p>
					<?php es_attr_e( 'The count for all IP addresses has been reset successfully.', 'whtp' ); ?>
				</p>
			</div>
			<?php
		else :
		?>
			<div class="update-nag notice notice-warning">
				<p>
					<?php es_attr_e( 'Failed to reset count for all IP addresses', 'whtp' ); ?>
				</p>
			</div>
			<?php
		endif;
...
<div class="wrap">	
	<h2 class="mdl-card__title-text">
		<?php es_attr_e( 'Who Hit The Page Hit Counter - Visiting IP Addresses', 'whtp' ); ?>
	</h2>
	<p>
		<?php es_attr_e( 'View the raw hit counter information. This page lists all the IP addresses that have visited your pages.', 'whtp' ); ?>
	</p>
